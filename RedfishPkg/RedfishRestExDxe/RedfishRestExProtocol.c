  ...
  )
{
  ...
  RESTEX_SEND_CHUNK_PROCESS     SendChunkProcess;
  RESTEX_SEND_NON_CHUNK_PROCESS SendNonChunkProcess;
  EFI_HTTP_MESSAGE       ChunkTransferRequestMessage;

  Status            = EFI_SUCCESS;
  ResponseData      = NULL;
  IsGetChunkedTransfer = FALSE;
  SendChunkProcess = RestExSendChunkNone;
  SendNonChunkProcess = RestExSendNonChunkNone;
...
ReSendRequest:;
  ...
  if (ItsWrite == TRUE) {
    ...
    //
    if (SendChunkProcess > RestExSendChunkNone) {
      //
      ...
      } while (SendChunkProcess == RestExSendChunkContent || SendChunkProcess == RestExSendChunkEndChunk);
    } else {
      //
      ...
      Status = HttpIoSendRequest(
                 &(Instance->HttpIo),
                 (SendNonChunkProcess == RestExSendNonChunkContent)? NULL: RequestMessage->Data.Request,
                 (SendNonChunkProcess == RestExSendNonChunkContent)? 0: RequestMessage->HeaderCount,
                 (SendNonChunkProcess == RestExSendNonChunkContent)? NULL: RequestMessage->Headers,
                 (SendNonChunkProcess == RestExSendNonChunkHeaderZeroContent)? 0: RequestMessage->BodyLength,
                 (SendNonChunkProcess == RestExSendNonChunkHeaderZeroContent)? NULL: RequestMessage->Body
                 );
    }
  } else {
    //
  ...
  if (ResponseData->Response.StatusCode == HTTP_STATUS_200_OK) {
    ...
    if (SendChunkProcess == RestExSendChunkHeaderZeroContent) {
    ...
    if (SendNonChunkProcess == RestExSendNonChunkHeaderZeroContent) {
