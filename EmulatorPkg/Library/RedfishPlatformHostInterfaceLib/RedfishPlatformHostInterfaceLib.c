...
/**
  ...
  @param[in] MacAddress      Pointer to retrieve MAC address
...
/**
  ...
  @param[in] RedfishProtocolData  Pointer to retrieve the first REDFISH_OVER_IP_PROTOCOL_DATA.
  @param[in] RedfishProtocolDataSize  Size of REDFISH_OVER_IP_PROTOCOL_DATA.
...
)
{
  ...
  DEBUG ((EFI_D_INFO, "%a: GetRedfishRecordFromVariable() - %r\n", __FUNCTION__, Status));
  if (!EFI_ERROR (Status)) {
