  ...
  )
{
  ...
  while (((StringId + X_UEFI_REDFISH_STRING_ARRAY_ENTRY_NUMBER) / X_UEFI_REDFISH_STRING_ARRAY_ENTRY_NUMBER) > XuefiRedfishStringDatabase->StringsArrayBlocks) {
    Status = NewRedfishXuefiStringArray (FormsetPrivate, XuefiRedfishStringDatabase);
    if (EFI_ERROR (Status)) {
      ...
      case EFI_HII_PACKAGE_STRINGS:
        ...
        if (StringIdMapIsBuilt == FALSE) {
          if (AsciiStrStr (HiiStringPackageHeader->Language, X_UEFI_SCHEMA_PREFIX) == NULL) {
            DEBUG ((REDFISH_PLATFORM_CONFIG_DEBUG, "  No need to build x-uefi-redifsh string ID map for HII language %a\n", HiiStringPackageHeader->Language));
          } else {
