...
/*
  ...
  @param[out] ChildHandle       The handle to receive the create child.
...
)
{
  ...
  if (Image == NULL ||
      AccessMode >= EFI_REST_EX_SERVICE_ACCESS_MODE_MAX ||
      ConfigType >= EFI_REST_EX_CONFIG_TYPE_MAX ||
      ServiceType >= EFI_REST_EX_SERVICE_TYPE_MAX ||
  ...
  *ChildInstanceHandle = NULL;
  ...
  while (NoBuffer != 0) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      //
      switch (ConfigType) {
        case EFI_REST_EX_CONFIG_TYPE_HTTP:
          LenOfConfig = sizeof (EFI_REST_EX_HTTP_CONFIG_DATA);
          break;

        case EFI_REST_EX_CONFIG_TYPE_UNSPECIFIC:
